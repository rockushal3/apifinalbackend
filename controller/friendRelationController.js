...
exports.getrequest = (req, res) => {
    ...
    friendrelation.find({ user_id_2: req.params._id, Status: "Requested" }).populate('user_id_1').populate('user_id_2')
    .then(function (getfriend) {
        res.send(getfriend)
    }).catch(function (e) {
        res.send(e)
    })
...
exports.getAllFriends = (req, res) => {
    ...
    friendrelation.find({ $or: [{ user_id_2: req.params._id, Status: "Friends" }, { user_id_1: req.params._id, Status: "Friends" }] })
    .populate('user_id_1').populate('user_id_2').then(function (getfriend) {
        res.send(getfriend)
    }).catch(function (e) {
        res.send(e)
    })
